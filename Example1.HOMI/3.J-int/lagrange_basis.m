...
% returns the lagrange interpolant basis and its gradients w.r.t the parent coordinate system.
%
%         [N(xi),dNdxi(xi)] = lagrange_basis(type,coord,dim)
%
    ...
    case 'Q8'
        ...
        if size(coord,2) < 2
            ...
            dNdxi=1/4*[(1-eta)*(2*xi+eta), (1-xi)*(2*eta+xi);
                ...
                (1+eta)*(2*xi+eta), (1+xi)*(2*eta+xi);
                (1+eta)*(2*xi-eta), (1-xi)*(2*eta-xi);
                -4*xi*(1-eta),      -2*(1-xi*xi);
                2*(1-eta*eta),     -4*eta*(1+xi);
                -4*xi*(1+eta),       2*(1-xi*xi);
                -2*(1-eta*eta),     -4*eta*(1-xi)];
        end
...
I = eye(dim); %生成dim*dim的单位阵，二维问题dim=2
Nv = [];
for i=1:size(N,1)
    Nv = [Nv;I*N(i)]; %形成dim维的形函数矩阵
end
